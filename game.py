...
class Game:
    ...
    def nextChapter(self, chapter):
        ...
        for ninjaGirls in leveling.allNinjaGirls:
            ninjaGirl1 = ninjaGirl.NinjaGirl(self, ninjaGirls['WallPosition'], ninjaGirls['DelayStart'], ninjaGirls['FreqSecs'], ninjaGirls['Direction'])
    ...
    def showGameOverScreen(self):
        self.screen = pg.display.set_mode((settings.SCREENWIDTH,settings.SCREENHEIGHT))
        self.drawText(self.screen, settings.TEXT_FONT, "THATS IT",42,settings.SCREENWIDTH*.4 ,settings.SCREENHEIGHT*.45)
        self.drawText(self.screen, settings.TEXT_FONT, "Your DEAD!!",30,settings.SCREENWIDTH*.45,settings.SCREENHEIGHT*.65)
        pg.display.flip()
        pg.time.wait(1000)

    def showLevelStartScreen(self,level):
        self.screen = pg.display.set_mode((settings.SCREENWIDTH,settings.SCREENHEIGHT))
        self.drawText(self.screen, settings.TEXT_FONT, "GET READY",42,settings.SCREENWIDTH*.4 ,settings.SCREENHEIGHT*.45)
        self.drawText(self.screen, settings.TEXT_FONT, "Level {:02d}".format(level),30,settings.SCREENWIDTH*.45,settings.SCREENHEIGHT*.65)
        pg.display.flip()
        pg.time.wait(1000)

    def showBonusLife(self):
        self.screen = pg.display.set_mode((settings.SCREENWIDTH,settings.SCREENHEIGHT))
        self.drawText(self.screen, settings.TEXT_FONT, "EXTRA LIFE",42,settings.SCREENWIDTH*.4 ,settings.SCREENHEIGHT*.45)
        pg.display.flip()
        pg.time.wait(1000)
